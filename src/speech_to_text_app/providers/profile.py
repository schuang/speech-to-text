...
def _gemini_fields(config: AppConfig) -> tuple[ProviderField, ...]:
    return (ProviderField("gemini_api_key", "API Key", config.gemini_api_key, False, "GEMINI_API_KEY"),)
...
def _ollama_fields(config: AppConfig) -> tuple[ProviderField, ...]:
    return (ProviderField("ollama_base_url", "Base URL", config.ollama_base_url, False, "OLLAMA_BASE_URL"),)


def _build_gemini(config: AppConfig) -> SpeechProvider:
...
def _build_ollama(config: AppConfig) -> SpeechProvider:
...
_PROFILES = {
    "gemini": ProviderProfile("gemini", "Google Gemini", "gemini-3.5-flash-lite", _gemini_fields, _missing_environment_value("gemini_api_key", "GEMINI_API_KEY"), _build_gemini),
    ...
    "ollama": ProviderProfile("ollama", "Ollama", "gemma4:default", _ollama_fields, _missing_environment_value("ollama_base_url", "OLLAMA_BASE_URL"), _build_ollama),
}


def provider_profile(provider: str) -> ProviderProfile:
    return _PROFILES.get(provider.strip().lower(), _PROFILES["gemini"])
